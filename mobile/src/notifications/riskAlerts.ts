...
export const RISK_ALERT_CATEGORY_ID = "risk-alert";
export const VIEW_ANALYSIS_ACTION_ID = "view_analysis";
...
async function ensureRiskNotificationActions() {
  if (Platform.OS === "web") return;
  await Notifications.setNotificationCategoryAsync(RISK_ALERT_CATEGORY_ID, [
    {
      identifier: VIEW_ANALYSIS_ACTION_ID,
      buttonTitle: "Ver análisis",
      options: {
...
export async function registerRiskAlerts(requestPermission = true): Promise<RegistrationResult> {
  ...
  await ensureRiskNotificationActions();
